...
//---------------------------------------------------------------------------
static uint64_t s_xorshift_state[2];
void TVPGetRandomBits128(void* dest)
{
    if (s_xorshift_state[0] == 0 && s_xorshift_state[1] == 0)
    {
        // 首次初始化
        s_xorshift_state[0] = (uint64_t)time(NULL) ^ 0x9E3779B97F4A7C15ULL;
        s_xorshift_state[1] = (uint64_t)clock() << 32 ^ 0xBF58476D1CE4E5B9ULL;
    }

    uint64_t s1 = s_xorshift_state[0];
    uint64_t s0 = s_xorshift_state[1];

    s_xorshift_state[0] = s0;
    s1 ^= s1 << 23;
    s_xorshift_state[1] = s1 ^ s0 ^ (s1 >> 17) ^ (s0 >> 26);

    uint64_t* out = (uint64_t*)dest;
    out[0] = s_xorshift_state[0];
    out[1] = s_xorshift_state[1];
}
...
//---------------------------------------------------------------------------
// Text stream is used by TJS's Array.save, Dictionary.saveStruct etc. to input / output text files.
//---------------------------------------------------------------------------
...
tTJSNC_Debug::tTJSNC_Debug() : tTJSNativeClass(TJS_N("Debug"))
{
    ...
    //----------------------------------------------------------------------
    TJS_BEGIN_NATIVE_METHOD_DECL(/*func. name*/ logAsWarning)
    {
        if (numparams < 1)
            return TJS_E_BADPARAMCOUNT;
    ...
    TJS_END_NATIVE_STATIC_METHOD_DECL(/*func. name*/ logAsWarning)
    //----------------------------------------------------------------------
