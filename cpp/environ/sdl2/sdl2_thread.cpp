...
{
    Terminated = true;
    SDL_CondBroadcast(THR_IMPL->cond);
...
void tTVPThread::Sleep(unsigned int milliseconds)
{
    if (IsCurrentThread())
    {
        ...
        SDL_CondWaitTimeout(THR_IMPL->cond, THR_IMPL->mutex, (int)milliseconds);
...
{
    return (SDL_ThreadID() == SDL_GetThreadID(THR_IMPL->thread));
}
//---------------------------------------------------------------------------
...
tTVPThreadEvent::tTVPThreadEvent()
{
    ...
    EVT_IMPL->cond = SDL_CreateCond();
    EVT_IMPL->mutex = SDL_CreateMutex();
}

tTVPThreadEvent::~tTVPThreadEvent()
{
    SDL_DestroyCond(EVT_IMPL->cond);
...
bool tTVPThreadEvent::WaitFor(int timeout)
{
    ...
    bool result;
    if (timeout != 0)
        result = SDL_CondWaitTimeout(EVT_IMPL->cond, EVT_IMPL->mutex, (int)timeout) == 0;
    else
    {
        SDL_CondWait(EVT_IMPL->cond, EVT_IMPL->mutex);
        result = true;
    }
    SDL_UnlockMutex(EVT_IMPL->mutex);
    return result;
...
// tTVPCondition
//---------------------------------------------------------------------------
tTVPCondition::tTVPCondition() : _impl(SDL_CreateCond())
{
}
tTVPCondition::~tTVPCondition()
{
    SDL_DestroyCond((SDL_cond*)_impl);
}
void tTVPCondition::notify_one()
{
    SDL_CondSignal((SDL_cond*)_impl);
}
void tTVPCondition::notify_all()
{
    SDL_CondBroadcast((SDL_cond*)_impl);
}
void tTVPCondition::Wait(tTJSCriticalSection& cs)
{
    SDL_CondWait((SDL_cond*)_impl, (SDL_mutex*)cs._impl->_mutex);
}
bool tTVPCondition::WaitFor(tTJSCriticalSection& cs, unsigned int ms)
{
    return SDL_CondWaitTimeout((SDL_cond*)_impl, (SDL_mutex*)cs._impl->_mutex, (int)ms) == 0;
}
...
{
    SDL_AtomicLock(&splock);
}
void tTJSSpinLock::unlock()
{
    SDL_AtomicUnlock(&splock);
}
