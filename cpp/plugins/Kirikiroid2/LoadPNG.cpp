...
{
    bool result = false;
    if (type.StartsWith(TJS_N("png")))
        result = true;
    else if (type == TJS_N(".png"))
        result = true;
...
#define PNG_read_be32(a) \
    (((tjs_uint32)(a)[0] << 24) + ((tjs_uint32)(a)[1] << 16) + ((tjs_uint32)(a)[2] << 8) + \
     ((tjs_uint32)(a)[3]))
        ...
        user_struct->metainfopushcallback(user_struct->callbackdata, PNG_tag_vpag_w,
                                          ttstr((tjs_int)width));
        user_struct->metainfopushcallback(user_struct->callbackdata, PNG_tag_vpag_h,
                                          ttstr((tjs_int)height));
        switch (unit)
        {
            case PNG_OFFSET_PIXEL:
                user_struct->metainfopushcallback(user_struct->callbackdata, PNG_tag_vpag_unit,
                                                  PNG_tag_pixel);
                break;
...
//---------------------------------------------------------------------------
void TVPLoadPNG(void* formatdata,
                void* callbackdata,
                ...
                tTVPGraphicLoadMode mode)
{
    ...
    {
        // create png_struct
        png_ptr = png_create_read_struct_2(PNG_LIBPNG_VER_STRING, (png_voidp)NULL,
                                           (png_error_ptr)PNG_error, (png_error_ptr)PNG_warning,
                                           (png_voidp)NULL, (png_malloc_ptr)PNG_malloc,
                                           (png_free_ptr)PNG_free);
        // png_ptr = png_create_read_struct( PNG_LIBPNG_VER_STRING, (png_voidp)NULL,
        // (png_error_ptr)PNG_error, (png_error_ptr)PNG_warning );
        if (!png_ptr)
            TVPThrowExceptionMessage(TVPPNGLoadError, (const tjs_char*)TVPLibpngError);
        ...
        if (mode == glmPalettized)
        {
            ...
            if (bit_depth > 8)
                TVPThrowExceptionMessage(TVPPNGLoadError,
                                         (const tjs_char*)TVPUnsupportedColorTypePalette);
        ...
        if (width >= 65536 || height >= 65536)
        {
            ...
            TVPThrowExceptionMessage(TVPPNGLoadError, (const tjs_char*)TVPTooLargeImage);
        }
        ...
        sizecallback(callbackdata, width, height,
                     color_type == PNG_COLOR_TYPE_RGB_ALPHA ? gpfRGBA : gpfRGB);
        ...
        if (png_get_interlace_type(png_ptr, info_ptr) == PNG_INTERLACE_NONE)
        {
            ...
            else
            {
                for (i = 0; i < height; i++)
                {
                    ...
                    TVPBLConvert24BitTo8Bit((tjs_uint8*)scanline, (tjs_uint8*)image, width);
                    scanlinecallback(callbackdata, -1);
                }
            }
#else
        ...
        else
        {
            ...
            for (i = 0; i < height; i++)
            {
                ...
                {
                    TVPBLConvert24BitTo8Bit((tjs_uint8*)scanline, (tjs_uint8*)row_pointers[i],
                                            width);
                }
                scanlinecallback(callbackdata, -1);
            }
 ...
 */
void TVPSaveAsPNG(void* formatdata,
                  ...
                  iTJSDispatch2* meta)
{
    ...
    try
    {
        ...
        png_set_write_fn(png_ptr, (png_voidp)stream, (png_rw_ptr)PNG_write_write,
                         (png_flush_ptr)PNG_write_flash);

        png_set_IHDR(png_ptr, info_ptr, width, height, 8,
                     (bpp == 32 ? PNG_COLOR_TYPE_RGB_ALPHA : PNG_COLOR_TYPE_RGB),
