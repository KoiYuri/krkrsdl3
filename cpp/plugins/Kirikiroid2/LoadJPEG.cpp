...
{
    bool result = false;
    if (type.StartsWith(TJS_N("jpg")))
        result = true;
    else if (type == TJS_N(".jpg"))
        result = true;
    else if (type == TJS_N(".jpeg"))
        result = true;
    else if (type == TJS_N(".jif"))
        result = true;
    // quality 1 - 100
    ...
    if (result && dic)
    {
        ...
        TVPExecuteExpression(
            TJS_N("(const)%[")
                TJS_N("\"quality\"=>(const)%[\"type\"=>\"range\",\"min\"=>1,\"max\"=>100,\"desc\"=>"
                      "\"100 is high quality, 1 is low quality\",\"default\"=>90],")
                    TJS_N("\"subsampling\"=>(const)%[\"type\"=>\"select\",\"items\"=>(const)[\"4:2:"
                          "0\",\"4:2:2\",\"4:4:4\"],\"desc\"=>\"subsampling\",\"default\"=>0],")
                        TJS_N("\"dct\"=>(const)%[\"type\"=>\"select\",\"items\"=>(const)[\"islow\","
                              "\"ifast\",\"float\"],\"desc\"=>\"DCT method\",\"default\"=>0],")
                            TJS_N("\"progressive\"=>(const)%[\"type\"=>\"boolean\",\"desc\"=>\"100 "
                                  "is high quality, 1 is low quality\",\"default\"=>true]")
                                TJS_N("]"),
            NULL, &result);
...
//---------------------------------------------------------------------------
void TVPLoadJPEG(void* formatdata,
                 void* callbackdata,
                 ...
                 tTVPGraphicLoadMode mode)
{
    ...
    try
    {
        ...
        tjDecompress2(jpegDecompressor, jpegBuf, jpegSize, buffer, width, width * numcolor, height,
                      pixelFormat, flags);
        if (mode == glmGrayscale)
        {
        ...
        else
        {
            for (int y = 0; y < height; y++)
            {
                ...
                if (!scanline)
                    break;
                memcpy(scanline, (const void*)&buffer[y * width * sizeof(tjs_uint32)],
...
METHODDEF(void)
JPEG_write_stream(
    j_compress_ptr cinfo, JOCTET* buffer, int bufsize, int* outsize, tTJSBinaryStream* stream)
{
    ...
    if (cinfo->dest == 0)
    {
        cinfo->dest = (struct jpeg_destination_mgr*)(*cinfo->mem->alloc_small)(
            (j_common_ptr)cinfo, JPOOL_PERMANENT, sizeof(stream_destination_mgr));
    }
 ...
 */
void TVPSaveAsJPG(void* formatdata,
