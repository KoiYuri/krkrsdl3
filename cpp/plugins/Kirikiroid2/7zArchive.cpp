...
#define SEVENZIP_CACHEBUFF_SIZE 64 * 1024
static ISzAlloc allocImp = {[](ISzAllocPtr p, size_t size) -> void* { return malloc(size); },
...
public:
    ...
    Byte cache[SEVENZIP_CACHEBUFF_SIZE] = {0};

public:
    SevenZipStreamWrap(tTJSBinaryStream* st) : _stream(st)
    {
        archiveStream.Host = this;
        archiveStream.Read = [](ISeekInStreamPtr p, void* buf, size_t* size) -> SRes
        { return ((CSeekInStream*)p)->Host->StreamRead(buf, size); };
        archiveStream.Seek = [](ISeekInStreamPtr p, Int64* pos, ESzSeek origin) -> SRes
        { return ((CSeekInStream*)p)->Host->StreamSeek(pos, origin); };
        LookToRead2_CreateVTable(&lookStream, false);
        lookStream.realStream = &archiveStream;
        lookStream.buf = cache;
        lookStream.bufSize = SEVENZIP_CACHEBUFF_SIZE;
        SzArEx_Init(&db);
        if (!g_CrcTable[1])
            CrcGenerateTable();
    }
    ...
    {
        size_t readSize = _stream->Read(buf, *size);
...
public:
    ...
    virtual tTJSBinaryStream* CreateStreamByIndex(tjs_uint idx)
    {
        ...
        if (folder.NumCoders == 1)
        {
            ...
            if (folder.Coders[0].MethodID == 0 && inSize == fileSize)
            {
        ...
        SRes res = SzArEx_Extract(&db, &lookStream.vt, fileIndex, &blockIndex, &outBuffer,
                                  &outBufferSize, &offset, &outSizeProcessed, &allocImp, &allocImp);
        if (res != SZ_OK || outBuffer == nullptr || outSizeProcessed == 0)
            return nullptr;
    ...
    bool Open(bool normalizeFileName)
    {
        ...
        for (int i = 0; i < db.NumFiles; i++)
        {
            ...
            std::vector<tjs_wchar> chw(len + 1);
            SzArEx_GetFileNameUtf16(&db, i, (UInt16*)chw.data());
            chw[len] = 0;
            int chLen = TVPWideCharToUtf8String(chw.data(), NULL);
            ttstr filename;
            TVPWideCharToUtf8String(chw.data(), filename.AllocBuffer(chLen));
            filename.FixLen();
