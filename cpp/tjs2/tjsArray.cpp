...
public:
    bool operator()(const tTJSVariant& lhs, const tTJSVariant& rhs) const
    {
        if (lhs.Type() == tvtString && rhs.Type() == tvtString)
            return (lhs < rhs).operator bool();
        return static_cast<ttstr>(lhs) < rhs;
    }
};
...
public:
    ...
    bool operator()(const tTJSVariant& lhs, const tTJSVariant& rhs) const
    {
        ...
        tjs_error hr;
        tTJSVariant* param[] = {const_cast<tTJSVariant*>(&lhs), // note that doing cast to
                                                                // non-const pointer
                                const_cast<tTJSVariant*>(&rhs)};

        hr = Closure.FuncCall(0, nullptr, nullptr, &result, 2, param, nullptr);
...
tTJSArrayClass::tTJSArrayClass() : tTJSNativeClass(TJS_N("Array"))
{
        ...
        if (param[0]->Type() == tvtObject)
        {
            ...
            if (clo.Object)
            {
                if (TJS_SUCCEEDED(clo.Object->NativeInstanceSupport(
                        TJS_NIS_GETINSTANCE, tTJSNC_RegExp::ClassID, (iTJSNativeInstance**)&re)))
                {
                    // param[0] is regexp
    ...
    TJS_BEGIN_NATIVE_METHOD_DECL(/* func.name */ pack)
    {
        ...
        if (numparams < 1)
            return TJS_E_BADPARAMCOUNT;
        if (result)
            return TJSOctetPack(param, numparams, ni->Items, result);
...
//----------------------------------------------------------------------
// same as count
TJS_BEGIN_NATIVE_PROP_DECL(length){TJS_BEGIN_NATIVE_PROP_GETTER{
    TJS_GET_NATIVE_INSTANCE(/* var. name */ ni, /* var. type */ tTJSArrayNI);
if (result)
    *result = (tTVInteger)(ni->Items.size());
return TJS_S_OK;
...
//---------------------------------------------------------------------------
tjs_error tTJSArrayNI::Construct(tjs_int numparams, tTJSVariant** params, iTJSDispatch2* tjsobj)
{
...
void tTJSArrayNI::Assign(iTJSDispatch2* dsp)
{
    ...
    if (TJS_SUCCEEDED(dsp->NativeInstanceSupport(TJS_NIS_GETINSTANCE, ClassID_Array,
...
//---------------------------------------------------------------------------
tjs_error tTJSArrayNI::tDictionaryEnumCallback::FuncCall(tjs_uint32 flag,
                                     ...
                                     const ttstr& indentstr)
{
    ...
    for (i = Items.begin(); i != Items.end(); i++)
    {
        ...
        if (type == tvtObject)
        {
            ...
            SaveStructuredDataForObject(clo.SelectObjectNoAddRef(), stack, stream, indentstr2);
        }
        else
        {
                                              ...
                                              const ttstr& indentstr)
{
    ...
    tTJSArrayNI* arrayni = NULL;
    if (dsp && TJS_SUCCEEDED(dsp->NativeInstanceSupport(
                   TJS_NIS_GETINSTANCE, TJSGetDictionaryClassID(), (iTJSNativeInstance**)&dicni)))
    {
        // dictionary
        stack.push_back(dsp);
    ...
    else if (dsp && TJS_SUCCEEDED(dsp->NativeInstanceSupport(TJS_NIS_GETINSTANCE, ClassID_Array,
                                                             (iTJSNativeInstance**)&arrayni)))
    {
        // array
        stack.push_back(dsp);
...
//---------------------------------------------------------------------------
void tTJSArrayNI::AssignStructure(iTJSDispatch2* dsp, std::vector<iTJSDispatch2*>& stack)
{
        ...
        try
        {
            ...
            for (i = arrayni->Items.begin(); i != arrayni->Items.end(); i++)
            {
                ...
                if (type == tvtObject)
                {
                    ...
                    else if (dsp && TJS_SUCCEEDED(dsp->NativeInstanceSupport(
...
#define ARRAY_GET_NI \
    tTJSArrayNI* ni; \
    if (TJS_FAILED(objthis->NativeInstanceSupport(TJS_NIS_GETINSTANCE, ClassID_Array, \
...
void tTJSArrayObject::Finalize()
{
    tTJSArrayNI* ni;
    if (TJS_FAILED(
            NativeInstanceSupport(TJS_NIS_GETINSTANCE, ClassID_Array, (iTJSNativeInstance**)&ni)))
    ...
    try
    {
        ...
        for (i = 0; i < ni->Items.size(); i++)
        {
            if (ni->Items[i].Type() == tvtObject)
            {
                ...
                tTJSVariantClosure clo = ni->Items[i].AsObjectClosureNoAddRef();
                clo.AddRef();
                if (clo.Object)
...
tjs_int tTJSArrayObject::Remove(tTJSArrayNI* ni, const tTJSVariant& ref, bool removeall)
{
    ...
    {
        // list objects up
        for (std::vector<tjs_int>::iterator i = todelete.begin(); i != todelete.end(); i++)
        {
            if (ni->Items[*i].Type() == tvtObject)
            {
                CheckObjectClosureRemove(ni->Items[*i]);
...
//---------------------------------------------------------------------------
tjs_error tTJSArrayObject::FuncCall(tjs_uint32 flag,
                                    const tjs_char* membername,
                                    tjs_uint32* hint,
                                    tTJSVariant* result,
                                    tjs_int numparams,
...
//---------------------------------------------------------------------------
tjs_error tTJSArrayObject::FuncCallByNum(tjs_uint32 flag,
                                         tjs_int num,
                                         tTJSVariant* result,
                                         tjs_int numparams,
                                         tTJSVariant** param,
...
//---------------------------------------------------------------------------
tjs_error tTJSArrayObject::PropGet(tjs_uint32 flag,
                                   const tjs_char* membername,
                                   tjs_uint32* hint,
                                   tTJSVariant* result,
                                   iTJSDispatch2* objthis)
...
//---------------------------------------------------------------------------
tjs_error tTJSArrayObject::PropGetByNum(tjs_uint32 flag,
                                        tjs_int num,
                                        tTJSVariant* result,
                                        iTJSDispatch2* objthis)
{
...
//---------------------------------------------------------------------------
tjs_error tTJSArrayObject::PropSet(tjs_uint32 flag,
                                   const tjs_char* membername,
                                   tjs_uint32* hint,
...
//---------------------------------------------------------------------------
tjs_error tTJSArrayObject::PropSetByVS(tjs_uint32 flag,
...
//---------------------------------------------------------------------------
tjs_error tTJSArrayObject::DeleteMember(tjs_uint32 flag,
                                        const tjs_char* membername,
                                        tjs_uint32* hint,
                                        iTJSDispatch2* objthis)
{
...
//---------------------------------------------------------------------------
tjs_error tTJSArrayObject::DeleteMemberByNum(tjs_uint32 flag, tjs_int num, iTJSDispatch2* objthis)
{
    if (!GetValidity())
        return TJS_E_INVALIDOBJECT;
...
//---------------------------------------------------------------------------
tjs_error tTJSArrayObject::Invalidate(tjs_uint32 flag,
                                      const tjs_char* membername,
                                      tjs_uint32* hint,
                                      iTJSDispatch2* objthis)
{
...
//---------------------------------------------------------------------------
tjs_error tTJSArrayObject::InvalidateByNum(tjs_uint32 flag, tjs_int num, iTJSDispatch2* objthis)
{
    if (!GetValidity())
        return TJS_E_INVALIDOBJECT;
...
//---------------------------------------------------------------------------
tjs_error tTJSArrayObject::IsValid(tjs_uint32 flag,
                                   const tjs_char* membername,
                                   tjs_uint32* hint,
                                   iTJSDispatch2* objthis)
{
...
//---------------------------------------------------------------------------
tjs_error tTJSArrayObject::IsValidByNum(tjs_uint32 flag, tjs_int num, iTJSDispatch2* objthis)
{
    if (!GetValidity())
        return TJS_E_INVALIDOBJECT;
...
//---------------------------------------------------------------------------
tjs_error tTJSArrayObject::CreateNew(tjs_uint32 flag,
                                     const tjs_char* membername,
                                     tjs_uint32* hint,
...
//---------------------------------------------------------------------------
tjs_error tTJSArrayObject::CreateNewByNum(tjs_uint32 flag,
                                          tjs_int num,
                                          iTJSDispatch2** result,
                                          tjs_int numparams,
                                          tTJSVariant** param,
...
//---------------------------------------------------------------------------
tjs_error tTJSArrayObject::IsInstanceOf(tjs_uint32 flag,
                                        const tjs_char* membername,
                                        tjs_uint32* hint,
...
//---------------------------------------------------------------------------
tjs_error tTJSArrayObject::IsInstanceOfByNum(tjs_uint32 flag,
                                             tjs_int num,
                                             const tjs_char* classname,
                                             iTJSDispatch2* objthis)
{
...
//---------------------------------------------------------------------------
tjs_error tTJSArrayObject::Operation(tjs_uint32 flag,
                                     const tjs_char* membername,
                                     tjs_uint32* hint,
                                     tTJSVariant* result,
                                     const tTJSVariant* param,
...
//---------------------------------------------------------------------------
tjs_error tTJSArrayObject::OperationByNum(tjs_uint32 flag,
                                          tjs_int num,
                                          tTJSVariant* result,
                                          const tTJSVariant* param,
                                          iTJSDispatch2* objthis)
