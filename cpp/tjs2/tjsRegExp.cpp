...
//---------------------------------------------------------------------------
void replace_regex(tTJSVariant** param,
                   tjs_int numparams,
                   tTJSNI_RegExp* _this,
                   ...
                   ttstr& res)
{
    ...
    int r = onig_search(_this->RegEx, (UChar*)s, (UChar*)send, (UChar*)s, (UChar*)send, region,
                        ONIG_OPTION_NONE);
    int offset = 0;
    if (r >= 0)
    { // match
...
//---------------------------------------------------------------------------
iTJSDispatch2* split_regex(const ttstr& target,
                           iTJSDispatch2* array,
                           tTJSNI_RegExp* _this,
                           bool purgeempty)
{
...
tTJSNC_RegExp::tTJSNC_RegExp()
  : tTJSNativeClass(TJS_N("RegExp")){
        ...
        TJS_BEGIN_NATIVE_MEMBERS(/*TJS class name*/ RegExp) TJS_DECL_EMPTY_FINALIZE_METHOD
            //----------------------------------------------------------------------
    ...
    */
    ...
    try
    {
        ...
        int r = onig_new(&(_this->RegEx), (UChar*)exprstart, (UChar*)(expr.c_str() + expr.length()),
                         flags & ((ONIG_OPTION_MAXBIT << 1) - 1), ONIG_ENCODING_UTF8,
...
void tTJSNC_RegExp::Compile(tjs_int numparams, tTJSVariant** param, tTJSNI_RegExp* _this)
{
    ...
    if (expr.IsEmpty())
        expr = TJS_N("(?:)"); // generate empty regular expression
    ...
    int r = onig_new(&(_this->RegEx), (UChar*)expr.c_str(), (UChar*)(expr.c_str() + expr.length()),
                     flags & ((ONIG_OPTION_MAXBIT << 1) - 1), ONIG_ENCODING_UTF8, ONIG_SYNTAX_PERL,
                     &einfo);
    if (r)
    {
        char s[ONIG_MAX_ERROR_MESSAGE_LEN];
...
bool tTJSNC_RegExp::Match(OnigRegion* region, const ttstr& target, tTJSNI_RegExp* _this)
{
    ...
    int r = onig_search(_this->RegEx, (UChar*)(target.c_str() + searchstart),
                        (UChar*)(target.c_str() + targlen), (UChar*)(target.c_str() + searchstart),
                        (UChar*)(target.c_str() + targlen), region, ONIG_OPTION_NONE);
    return r >= 0;
}
//---------------------------------------------------------------------------
bool tTJSNC_RegExp::Exec(OnigRegion* region, const ttstr& target, tTJSNI_RegExp* _this)
{
    bool matched = tTJSNC_RegExp::Match(region, target, _this);
    iTJSDispatch2* array =
        tTJSNC_RegExp::GetResultArray(matched, target.c_str() + _this->Start, _this, region);
