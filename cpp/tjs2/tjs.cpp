...
void tTJS::Dump(tjs_uint width) const
{
    ...
    TJS_snprintf(version, sizeof(version) / sizeof(tjs_char), TJS_N("TJS version %d.%d.%d (%s)"),
                 TJSVersionMajor, TJSVersionMinor, TJSVersionRelease, TJSCompiledDate);
    ...
    if (ScriptBlocks.size())
    {
        ...
        tjs_char buf[1024];
        TJS_snprintf(buf, sizeof(buf) / sizeof(tjs_char), TJS_N("Total %d script block(s)"),
                     ScriptBlocks.size());
        OutputToConsole(buf);
        OutputToConsole(TJS_N(""));
        ...
        for (i = ScriptBlocks.begin(); i != ScriptBlocks.end(); i++)
        {
            ...
            TJS_snprintf(buf, sizeof(buf) / sizeof(tjs_char),
                         TJS_N("\tCount of contexts      : %d"), n);
            OutputToConsole(buf);

            n = (*i)->GetTotalVMCodeSize();
            totalcodesize += n;
            TJS_snprintf(buf, sizeof(buf) / sizeof(tjs_char),
                         TJS_N("\tVM code area size      : %d words"), n);
            OutputToConsole(buf);

            n = (*i)->GetTotalVMDataSize();
            totaldatasize += n;
            TJS_snprintf(buf, sizeof(buf) / sizeof(tjs_char),
                         TJS_N("\tVM constant data count : %d"), n);
            OutputToConsole(buf);
        ...
        TJS_snprintf(buf, sizeof(buf) / sizeof(tjs_char),
                     TJS_N("Total count of contexts      : %d"), totalcontexts);
        OutputToConsole(buf);
        TJS_snprintf(buf, sizeof(buf) / sizeof(tjs_char),
                     TJS_N("Total VM code area size      : %d words"), totalcodesize);
        OutputToConsole(buf);
        TJS_snprintf(buf, sizeof(buf) / sizeof(tjs_char),
                     TJS_N("Total VM constant data count : %d"), totaldatasize);
        OutputToConsole(buf);
...
//---------------------------------------------------------------------------
void tTJS::ExecScript(const tjs_char* script,
                      tTJSVariant* result,
...
//---------------------------------------------------------------------------
void tTJS::ExecScript(const ttstr& script,
                      tTJSVariant* result,
                      iTJSDispatch2* context,
                      const ttstr* name,
                      tjs_int lineofs)
...
//---------------------------------------------------------------------------
void tTJS::EvalExpression(const tjs_char* expression,
                          tTJSVariant* result,
                          iTJSDispatch2* context,
                          const tjs_char* name,
                          tjs_int lineofs)
...
//---------------------------------------------------------------------------
void tTJS::EvalExpression(const ttstr& expression,
                          tTJSVariant* result,
                          iTJSDispatch2* context,
                          const ttstr* name,
                          tjs_int lineofs)
...
//---------------------------------------------------------------------------
// for Bytecode
void tTJS::LoadByteCode(const tjs_uint8* buff,
                        size_t len,
                        tTJSVariant* result,
                        iTJSDispatch2* context,
                        const tjs_char* name)
...
//---------------------------------------------------------------------------
bool tTJS::LoadByteCode(class tTJSBinaryStream* stream,
                        tTJSVariant* result,
                        iTJSDispatch2* context,
                        const tjs_char* name)
{
...
//---------------------------------------------------------------------------
iTJSTextReadStream* (*TJSCreateTextStreamForRead)(const tTJSString& name, const tTJSString& mode) =
    TJSDefCreateTextStreamForRead;
iTJSTextWriteStream* (*TJSCreateTextStreamForWrite)(
    const tTJSString& name, const tTJSString& mode) = TJSDefCreateTextStreamForWrite;
tTJSBinaryStream* (*TJSCreateBinaryStreamForRead)(const tTJSString& name, const tTJSString& mode) =
    TJSDefCreateBinaryStreamForRead;
tTJSBinaryStream* (*TJSCreateBinaryStreamForWrite)(const tTJSString& name, const tTJSString& mode) =
