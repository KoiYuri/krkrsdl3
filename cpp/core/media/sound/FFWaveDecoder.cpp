...
class FFWaveDecoder : public tTVPWaveDecoder // decoder interface
{
    ...
    // release in Clear()
    AVPacket Packet;
    tTJSBinaryStream* InputStream = NULL; // input stream
    ...
    void Clear()
    {
        ...
        if (avctx)
            avcodec_free_context(&avctx), avctx = nullptr;
    }
    bool ReadPacket();
...
bool FFWaveDecoder::SetPosition(tjs_uint64 samplepos)
{
    ...
    for (;;)
    {
        if (seek_temp < 0)
            seek_temp = 0;
        int ret = avformat_seek_file(FormatCtx, StreamIdx, seek_temp, seek_temp, seek_temp,
        ...
        audio_buf_index =
            (samplepos - frame->pts) /*/ av_q2d(AudioStream->time_base) / Format.SamplesPerSec*/;
...
bool FFWaveDecoder::SetStream(const ttstr& url)
{
    ...
    int bufSize = 32 * 1024;
    AVIOContext* pIOCtx =
        avio_alloc_context((unsigned char*)av_malloc(bufSize + AVPROBE_PADDING_SIZE),
    ...
    int ret = avcodec_parameters_to_context(avctx, FormatCtx->streams[StreamIdx]->codecpar);
    if (avctx->codec_type != AVMEDIA_TYPE_AUDIO || ret < 0)
    {
        return false;
    }
    ...
    Format.Channels = avctx->ch_layout.nb_channels;
    ...
    Format.TotalSamples =
        av_q2d(AudioStream->time_base) * AudioStream->duration * Format.SamplesPerSec;
