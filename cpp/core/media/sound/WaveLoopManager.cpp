...
{
    tjs_uint8 value[3];
    tTVPPCM24(tjs_int v) { operator=(v); }
...
//---------------------------------------------------------------------------
template<typename T>
static void TVPCrossFadeIntegerBlend(void* dest,
                                     void* src1,
                                     void* src2,
                                     tjs_int ratiostart,
                                     tjs_int ratioend,
...
{
    tjs_uint blend_step =
        (tjs_int)(((ratioend - ratiostart) * ((tjs_int64)1 << 32) / 100) / samples);
...
{
    volatile tTJSCriticalSectionHolder CS(FlagsCS);
    for (tjs_int i = 0; i < TVP_WL_MAX_FLAGS; i++)
        Flags[i] = 0;
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
void tTVPWaveLoopManager::Decode(void* dest,
                                 tjs_uint samples,
                                 tjs_uint& written,
                                 tTVPWaveSegmentQueue& segments)
{
    ...
    while (written != samples /* && Position < Format->TotalSamples*/)
    {
        ...
        if (!IgnoreLinks && GetNearestEvent(Position, link, false))
        {
            ...
            if (link.Smooth)
            {
                ...
                else if (!CrossFadeSamples)
                {
                    ...
                    try
                    {
                        tjs_int alloc_size = (before_count + after_count) * Format->BytesPerSample *
                                             Format->Channels;
                    ...
                    catch (...)
                    {
                        ...
                        if (CrossFadeSamples)
                            delete[] CrossFadeSamples, CrossFadeSamples = NULL;
                        if (src1)
                            delete[] src1;
                        if (src2)
                            delete[] src2;
                    ...
                    if (CrossFadeSamples)
                    {
                        ...
                        DoCrossFade(CrossFadeSamples + after_offset, src1 + after_offset,
                                    src2 + after_offset, after_count, 50, 100);
                        delete[] src1;
                        delete[] src2;
        ...
        if (one_unit > 0)
            give_up_count = 0; // reset give up count
        ...
        else
        {
            ...
            memcpy((void*)d,
                   CrossFadeSamples + CrossFadePosition * Format->BytesPerSample * Format->Channels,
                                          ...
                                          bool ignore_conditions)
{
    ...
    if (Links.size() == 0)
        return false; // there are no event
            ...
            if (Links[s].CondVar != -1)
            {
                ...
                switch (Links[s].Condition)
                {
                    case llcNone:
                        ...
                        break;
                    case llcEqual:
                        if (Links[s].RefValue == Flags[Links[s].CondVar])
                            match = true;
                        break;
                    case llcNotEqual:
                        if (Links[s].RefValue != Flags[Links[s].CondVar])
...
//---------------------------------------------------------------------------
void tTVPWaveLoopManager::GetLabelAt(tjs_int64 from,
                                     tjs_int64 to,
                                     std::deque<tTVPWaveLabel>& labels)
{
    volatile tTJSCriticalSectionHolder CS(FlagsCS);
    ...
    if (s < (int)Labels.size() - 1 && Labels[s].Position < from)
        s++;
...
//---------------------------------------------------------------------------
void tTVPWaveLoopManager::DoCrossFade(
    void* dest, void* src1, void* src2, tjs_int samples, tjs_int ratiostart, tjs_int ratioend)
{
    ...
    else
    {
        ...
        {
            TVPCrossFadeIntegerBlend<tTVPPCM8>(dest, src1, src2, ratiostart, ratioend, samples,
                                               Format->Channels);
        }
        else if (Format->BytesPerSample == 2)
        {
            TVPCrossFadeIntegerBlend<tjs_int16>(dest, src1, src2, ratiostart, ratioend, samples,
                                                Format->Channels);
        }
        else if (Format->BytesPerSample == 3)
        {
            TVPCrossFadeIntegerBlend<tTVPPCM24>(dest, src1, src2, ratiostart, ratioend, samples,
                                                Format->Channels);
        }
        else if (Format->BytesPerSample == 4)
        {
            TVPCrossFadeIntegerBlend<tjs_int32>(dest, src1, src2, ratiostart, ratioend, samples,
                                                Format->Channels);
        }
    }
}
                                             ...
                                             bool* is_rv_indirect)
{
    ...
    if (*p != ':')
        return false; // not expression
    p++;

    token = GetExpressionToken(p, &value);
    if (token != etLBracket)
        return false; // lvalue must form of '[' integer ']'
    token = GetExpressionToken(p, &value);
    if (token != etInteger)
        return false; // lvalue must form of '[' integer ']'
    lvalue = value;
    if (lvalue < 0 || lvalue >= TVP_WL_MAX_FLAGS)
    ...
    token = GetExpressionToken(p, &value);
    if (token != etEOE)
        return false; // excess characters
...
//---------------------------------------------------------------------------
tTVPWaveLoopManager::tExpressionToken tTVPWaveLoopManager::GetExpressionToken(
    const tTVPLabelCharType*& p, tjs_int* value)
{
...
bool tTVPWaveLoopManager::ReadInformation(char* p)
{
    ...
    if (*p != '#')
    {
        ...
        tjs_int64 start;
        tjs_int64 length;
        if (!GetInt64(p_length + 11, length))
            return false;
        if (!GetInt64(p_start + 10, start))
            return false;
        ...
        while (true)
        {
            ...
            else if (!TJS_strncasecmp(p, "Label", 5) && !isalpha(p[5]))
            {
                ...
                if (!*p)
                    return false;
                tTVPWaveLabel label;
                if (!ReadLabelInformation(p, label))
                    return false;
...
{
    if (!Decoder->DesiredFormat(format))
        return false;
    SetDecoder(Decoder);
    return true;
}
