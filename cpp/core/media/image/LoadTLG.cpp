...
//---------------------------------------------------------------------------
void TVPLoadTLG6(void* formatdata,
                 void* callbackdata,
                 tTVPGraphicSizeCallback sizecallback,
                 tTVPGraphicScanLineCallback scanlinecallback,
                 tTJSBinaryStream* src,
                 ...
                 bool palettized)
{
    ...
    try
    {
        ...
        pixelbuf =
            (tjs_uint32*)TJSAlignedAlloc(sizeof(tjs_uint32) * width * TVP_TLG6_H_BLOCK_SIZE + 1, 4);
        ...
        for (tjs_int y = 0; y < height; y += TVP_TLG6_H_BLOCK_SIZE)
        {
            ...
            for (tjs_int c = 0; c < colors; c++)
            {
                ...
                switch (method)
                {
                    case 0:
                        if (c == 0 && colors != 1)
                            TVPTLG6DecodeGolombValuesForFirst((tjs_int8*)pixelbuf, pixel_count,
                                                              bit_pool);
                        else
                            TVPTLG6DecodeGolombValues((tjs_int8*)pixelbuf + c, pixel_count,
                                                      bit_pool);
                        break;
                    default:
                        TVPThrowExceptionMessage(
                            TVPTLGLoadError, (const tjs_char*)TVPUnsupportedEntropyCodingMethod);
                }
            }
            ...
            for (int yy = y; yy < ylim; yy++)
            {
                ...
                if (main_count)
                {
                    int start = ((width < TVP_TLG6_W_BLOCK_SIZE) ? width : TVP_TLG6_W_BLOCK_SIZE) *
                                (yy - y);
                    TVPTLG6DecodeLine(prevline, curline, width, main_count, ft, skipbytes,
                ...
                if (main_count != x_block_count)
                {
                    ...
                    int start = ww * (yy - y);
                    TVPTLG6DecodeLineGeneric(prevline, curline, width, main_count, x_block_count,
                                             ft, skipbytes, pixelbuf + start,
...
// TLG loading handler
//---------------------------------------------------------------------------
static void TVPInternalLoadTLG(void* formatdata,
                               void* callbackdata,
                               tTVPGraphicSizeCallback sizecallback,
                               tTVPGraphicScanLineCallback scanlinecallback,
                               ...
                               tTVPGraphicLoadMode mode)
{
    ...
    {
        TVPLoadTLG6(formatdata, callbackdata, sizecallback, scanlinecallback, src, keyidx,
                    mode != glmNormal);
    }
    else
    {
                ...
                tTVPGraphicLoadMode mode)
{
    ...
    if (!memcmp("TLG0.0\x00sds\x1a\x00", mark, 11))
    {
        ...
        TVPInternalLoadTLG(formatdata, callbackdata, sizecallback, scanlinecallback,
