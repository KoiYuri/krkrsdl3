...
tTJSNC_PhaseVocoder::tTJSNC_PhaseVocoder()
  : tTJSNativeClass(TJS_N("PhaseVocoder")){
        ...
        TJS_BEGIN_NATIVE_MEMBERS(PhaseVocoder) TJS_DECL_EMPTY_FINALIZE_METHOD
            //----------------------------------------------------------------------
            // constructor/methods
            //----------------------------------------------------------------------
            TJS_BEGIN_NATIVE_CONSTRUCTOR_DECL(/*var.name*/ _this,
                                              /*var.type*/ tTJSNI_PhaseVocoder,
                                              /*TJS class name*/ PhaseVocoder){return TJS_S_OK;
}
...
//----------------------------------------------------------------------
TJS_BEGIN_NATIVE_PROP_DECL(overlap){TJS_BEGIN_NATIVE_PROP_GETTER{
    TJS_GET_NATIVE_INSTANCE(/*var. name*/ _this, /*var. type*/ tTJSNI_PhaseVocoder);
...
//----------------------------------------------------------------------
TJS_BEGIN_NATIVE_PROP_DECL(pitch){TJS_BEGIN_NATIVE_PROP_GETTER{
    TJS_GET_NATIVE_INSTANCE(/*var. name*/ _this, /*var. type*/ tTJSNI_PhaseVocoder);
...
//----------------------------------------------------------------------
TJS_BEGIN_NATIVE_PROP_DECL(time){TJS_BEGIN_NATIVE_PROP_GETTER{
    TJS_GET_NATIVE_INSTANCE(/*var. name*/ _this, /*var. type*/ tTJSNI_PhaseVocoder);
...
{
    if (PhaseVocoder)
        delete PhaseVocoder, PhaseVocoder = NULL;
    if (FormatConvertBuffer)
        delete[] FormatConvertBuffer, FormatConvertBuffer = NULL, FormatConvertBufferSize = 0;
}
//---------------------------------------------------------------------------
tjs_error tTJSNI_PhaseVocoder::Construct(tjs_int numparams,
                                         tTJSVariant** param,
...
//---------------------------------------------------------------------------
void tTJSNI_PhaseVocoder::Fill(float* dest,
                               tjs_uint samples,
                               tjs_uint& written,
                               tTVPWaveSegmentQueue& segments)
{
    ...
    else
    {
        ...
        if (FormatConvertBufferSize < buf_size)
        {
            // バッファを再確保
            if (FormatConvertBuffer)
                delete[] FormatConvertBuffer, FormatConvertBuffer = NULL;
...
//---------------------------------------------------------------------------
void tTJSNI_PhaseVocoder::Decode(void* dest,
                                 tjs_uint samples,
                                 tjs_uint& written,
                                 tTVPWaveSegmentQueue& segments)
{
    ...
    {
        // PhaseVocoder を作成
        tRisaPhaseVocoderDSP* pv =
            new tRisaPhaseVocoderDSP(Window, InputFormat.SamplesPerSec, InputFormat.Channels);
    ...
    while (samples > 0)
    {
        ...
        do
        {
            ...
            if (inputfree >= inputhopsize)
            {
                ...
                if (p2)
                    Fill(p2, (tjs_uint)p2len, filled, InputSegments), total += filled;
                if (total == 0)
                {
                    break;
                } // もうデータがない
