...
#define NCB_WARN_W(str) NCB_WARN(TJS_N(str))
...
struct ncbTypedefs
{
    ...
    {
        return (const_cast<tTJSVariant*>(&var))->Type();
    }
    ...
    template<bool F, typename A, typename B>
    struct TypeSelect
    {
        typedef B Result;
    };
    template<typename A, typename B>
    struct TypeSelect<true, A, B>
    {
        typedef A Result;
    };
    template<bool F, typename ERR>
    struct TypeAssert
    {
        typedef void Result;
    };
    template<typename ERR>
    struct TypeAssert<true, ERR>
    {
        typedef typename ERR::CompileError Result;
    };
};

/// サブクラスフラグ
template<class T>
struct ncbSubClassCheck
{
    enum
    {
        IsSubClass = false
...
template<>
struct ncbClassInfo<void>
{
};
template<class T>
typename ncbClassInfo<T>::InfoT ncbClassInfo<T>::_info;
...
public:
    ...
    static AdaptorT* GetAdaptor(iTJSDispatch2* obj, bool err = false)
    {
        ...
        {
            if (err)
                TVPThrowExceptionMessage(TJS_N("No instance."));
            return 0;
        }
        if (TJS_FAILED(obj->NativeInstanceSupport(TJS_NIS_GETINSTANCE, ClassInfoT::GetID(), &adp)))
        {
            if (err)
                TVPThrowExceptionMessage(TJS_N("Invalid instance type."));
            return 0;
        }
    ...
    static bool SetAdaptorWithNativeInstance(iTJSDispatch2* obj,
                                             NativeClassT* instance,
                                             bool err = false)
    {
        ...
        {
            if (err)
                TVPThrowExceptionMessage(TJS_N("Create adaptor failed."));
            return false;
        }
        adp->_instance = instance;
        ...
        {
            if (err)
                TVPThrowExceptionMessage(TJS_N("Adaptor registration failed."));
            return false;
        }
        return true;
    }
    ...
    static iTJSDispatch2* CreateAdaptor(NativeClassT* inst, bool sticky = false, bool err = false)
    {
        ...
        {
            if (err)
                TVPThrowExceptionMessage(TJS_N("No class object."));
            return 0;
        }
        ...
        {
            if (err)
                TVPThrowExceptionMessage(TJS_N("Can't create instance"));
            return 0;
        }
        AdaptorT* adp = GetAdaptor(obj, err);
        if (adp)
...
#define NCB_INNER_CONVERSION_SPECIALIZATION \
    template<> \
    struct ncbTypeConvertor::Conversion<void, void> \
    { \
        enum \
        { \
            Exists = false, \
    ...
    struct Stripper<T*>
    {
        typedef typename Stripper<T>::Type Type;
    };
    template<typename T>
    struct Stripper<T&>
    {
    ...
    struct DirectCopy
    {
        ...
        inline void operator()(DST& dst, SRC const& src) const
        {
            dst = src;
        }
    };
    ...
    struct CastCopy
    {
        ...
        inline void operator()(DST& dst, SRC const& src) const
        {
            dst = static_cast<DST>(static_cast<CAST>(src));
        }
    };
    ...
    protected:
        /// ３項演算子
        template<bool EXP, class THEN, class ELSE>
        struct ifelse
        {
            typedef ELSE Type;
        };
        ...
        struct directSelect
        {
            typedef typename ifelse<Conversion<SRC, DST>::Exists,
                                    DirectCopy,
                                    NCB_COMPILE_ERROR_NoImplement>::Type Type;
        };
    };
    ...
    private:
        ...
        {
            typedef typename ifelse<DstSpecialT::Exists,
                                    typename DstSpecialT::Type,
                                    typename SrcSpecialT::Type>::Type Type;
        };
        typedef typename ifelse<SrcSpecialT::Exists || DstSpecialT::Exists,
                                specialSelect,
                                directSelect<SRC, DST>>::Type select;
...
#define NCB_TYPECONV_MAPSET(mapsel, type, conv, mod) \
    template<> \
    struct ncbTypeConvertor::SpecialMap<type, mapsel> \
    { \
        enum \
        { \
            Exists = true, \
    ...
    struct ToNChar
    {
        ...
        inline void operator()(DST& dst, tTJSVariant const& src)
        {
            if (ncbTypedefs::GetVariantType(src) == tvtString)
            {
                ...
                _nstr = new tjs_char[s.length() + 1];
                std::memcpy(_nstr, s.c_str(), s.length());
                _nstr[s.length()] = 0;
            }
...
#define NCB_TYPECONV_WIDE_STRING(type) \
    NCB_TYPECONV_SRCMAP_SET(type, ncbTypeConvertor::CastCopy<tjs_wchar const*>, false); \
    NCB_TYPECONV_DSTMAP_SET(type, ncbWideCharConvertor::ToWChar, false)

NCB_TYPECONV_WIDE_STRING(tjs_wchar const*);
...
struct ncbStringConvertor
{
    ...
    inline void set(tTJSString const& str, DefsT::NumTag<sizeof(tjs_char)>) { _temp = str.c_str(); }
...
#define NCB_SET_TOVARIANT_CONVERTOR(type, convertor) \
    template<> \
    struct ncbTypeConvertor::SelectConvertorType<type, tTJSVariant> \
    { \
        typedef convertor Type; \
    }

#define NCB_SET_TOVALUE_CONVERTOR(type, convertor) \
    template<> \
    struct ncbTypeConvertor::SelectConvertorType<tTJSVariant, type> \
    { \
        typedef convertor Type; \
    }
...
struct ncbPropAccessor
{
    ...
    {
        VariantT var;
        _obj->PropGet(0, TJS_N("count"), 0, &var, _obj);
        return (CountT)var;
    }
    ...
    template<typename TargetT>
    TargetT GetValue(
        iTJSDispatch2* obj, KeyT key, DefsT::Tag<TargetT> const& tag, FlagsT f = 0, HintT hint = 0)
    {
        VariantT var;
    ...
    template<typename TargetT>
    bool SetValue(iTJSDispatch2* obj,
                  KeyT key,
                  TargetT const& val,
                  FlagsT f = TJS_MEMBERENSURE,
                  HintT hint = 0)
    {
        VariantT var;
        _toVariant(var, val);
...
#define FOREACH \
    tjs_error FuncCall(tjs_uint32 flag, const tjs_char* membername, tjs_uint32* hint, \
                       tTJSVariant* result, FOREACH_COMMA_EXT(FCALL_PRM_EXT)) \
    { \
        tTJSVariant* params[FOREACH_COUNT]; \
        FOREACH_SPACE_EXT(FCALL_SET_EXT) \
        return _obj->FuncCall(flag, membername, hint, result, FOREACH_COUNT, params, _obj); \
    }
#include FOREACH_INCLUDE
#undef FOREACH
#define FOREACH \
    tjs_error FuncCall(iTJSDispatch2* obj, tjs_uint32 flag, const tjs_char* membername, \
                       tjs_uint32* hint, tTJSVariant* result, FOREACH_COMMA_EXT(FCALL_PRM_EXT)) \
    { \
        tTJSVariant* params[FOREACH_COUNT]; \
        FOREACH_SPACE_EXT(FCALL_SET_EXT) \
...
#undef FCALL_SET_EXT
    ...
    tjs_error FuncCall(iTJSDispatch2* obj,
                       tjs_uint32 flag,
                       const tjs_char* membername,
                       tjs_uint32* hint,
                       tTJSVariant* result)
...
struct ncbNativeClassMethodBase : public tTJSDispatch
{
                           ...
                           iTJSDispatch2* objthis)
    {
        ...
        return (!membername && _name && !TJS_stricmp(classname, _name))
                   ? TJS_S_TRUE
...
private:
    ...
    struct paramsFunctor
    {
        ...
        {
            StrictResult =
                ncbTypeConvertor::Conversion<ResultConvT*, ncbStrictResultConvertor*>::Exists
        };
        ...
        struct TypeWrap<T const&>
        {
            ...
            static inline T const& Restore(Type t) { return *t; }
        };

        template<typename T>
        struct ArgsExtor
        {
            typedef typename ConvT::SelectConvertorType<VariantT, typename TypeWrap<T>::Type>::Type
                Type;
        };
        typedef CallerT::tArgsExtract<ArgsExtor, ARGS, ARGC> ArgsConvT;
        /* ArgsConvT は以下のように展開される：
                struct ArgsConvT {
                        ncbToValueConvertor<ARGS::Arg1Type> t1; // 一番目の引数の
           ncbToValueConvertor ncbToValueConvertor<ARGS::Arg2Type> t2; // 二番目の
                          :
                        ncbToValueConvertor<ARGS::Arg[ARGC]Type> tARGC; // ARGC番目の
                }; */

        /// constructor
        paramsFunctor(VariantT* r, tjs_int n, VariantT const* const* p)
          : _numparams(n),
            _result(r),
            _param(p)
        {
        ...
        inline T operator()(CallerT::tNumTag<N> const& /*index*/,
                            CallerT::tTypeTag<T> const& /*type*/)
        {
            typedef typename TypeWrap<T>::Type ParamT;
            ParamT ret;
            // N番目の ncbToValueConvertor を取り出して変換
            (CallerT::tArgsSelect<ArgsConvT, N>::Get(_aconv))(
                ret, (_numparams >= N) ? *(_param[N - 1]) : VariantT());
        ...
        // StrictResult = true
        template<typename ResultT>
        inline bool SetResult(ResultT r,
    ...
    struct paramsFunctorWithInstance : public FUNCT
    {
        ...
        typedef tTJSVariant VariantT;
        paramsFunctorWithInstance(VariantT* r, tjs_int n, VariantT const* const* p, ClassT* inst)
          : BaseT(r, n + 1, p - 1),
            _inst(inst)
        {
        }
        template<int N, typename T>
        inline T operator()(CallerT::tNumTag<N> const& idx, CallerT::tTypeTag<T> const& tag)
        {
            return BaseT::operator()(idx, tag);
        }
        template<typename T>
        inline T operator()(CallerT::tNumTag<1> const&, CallerT::tTypeTag<T> const&)
        {
            return _inst;
        }
        template<typename ResultT>
...
private:
    ...
    struct nativeInstanceGetterBase
    {
        ...
        inline ClassT* GetNativeInstance(DispatchT objthis)
        {
            ...
            if (!r)
                SetError(TJS_E_NATIVECLASSCRASH);
            return r;
        }
    ...
    // ダミー用
    typedef struct dummyGetter
    {
        dummyGetter() {}
    } const noInstanceGetter;

    //--------------------------------------
protected:
    /// メソッド/コンストラクタ呼び出しにtry/catchを挟み込むためのヘルパテンプレート
    template<bool IsAny>
    struct invokeHookAll
    {
        template<typename T>
    ...
    struct invokeHookClass
    {
        template<typename T>
        static inline typename T::ResultT Do(T& t)
        {
            return invokeHookAll<T::Hook>::Do(t);
        }
    };
...
protected:
    ...
    protected:
        ...
        ResultT CallInvoke(MethodT const& m,
                           ClassT* inst,
                           DefsT::Tag<FunctorT>,
                           DefsT::NumTag<ivsMethod>) const
        {
            return CallerT::Invoke(FunctorT(_result, _numparams, _param), m, inst) ? TJS_S_OK
                                                                                   : TJS_E_FAIL;
        }
        // Proxy 用
        template<typename MethodT, class ClassT, class FunctorT>
        ...
        {
            return CallerT::Invoke(FunctorT(_result, _numparams, _param, inst), m, inst)
                       ? TJS_S_OK
                       : TJS_E_FAIL;
        }
        // コンストラクタ用
        template<typename MethodT, class ClassT, class FunctorT>
        ResultT CallInvoke(MethodT const&,
                           ClassT* inst,
                           DefsT::Tag<FunctorT>,
                           DefsT::NumTag<ivsConstructor>) const
        {
            try
            {
                ...
                if (!(inst =
                          CallerT::Factory(fnct, CallerT::tTypeTag<ClassT>(),
                                           CallerT::tTypeTag<typename FunctorT::TraitsT::ArgsT>())))
                {
                    TVPThrowExceptionMessage(TJS_N("NativeClassInstance creation faild."));
                    return TJS_E_FAIL;
                }
        ...
        // インスタンスなし
        template<class ANY>
        ResultT GetInstance(ANY**, noInstanceGetter&) const
        {
            return TJS_S_OK;
        }
    };
    ...
    struct doInvoke : public doInvokeBase
    {
        ...
        doInvoke(MethodT const& m, RetT r, NumT n, ArgsT p, ObjT o)
          : doInvokeBase(r, n, p, o),
            _m(m)
        {
        }
        ...
        {
            // 引き数の個数が少ない場合はエラー
            if (_numparams < SelectorT::ArgsCount)
            ...
            return CallInvoke(_m, inst, DefsT::Tag<FunctorT>(),
                              DefsT::NumTag<SelectorT::InvokeSelect>());
        }
        inline ResultT Invoke() const { return invokeHookClass<RefClassT>::Do(*this); }
...
public:
    struct InvokeType
    {
        ...
        struct ivtBridge
        {
            ...
            typedef
                typename ncbTypeConvertor::Stripper<typename traits<METHOD>::ResultT>::Type TargetT;
        };
        ...
        struct InvokeCommand
        {
            ...
            typedef
                typename DefsT::TypeSelect<(DefsT::TypeEqual<ClassT, void>::Result),
                                           noInstanceGetter,
                                           nativeInstanceGetter<RefClassT>>::Result GetInstanceT;
            // GetInstanceT=     (ClassT==void) ?         noInstanceGetter:
            // nativeInstanceGetter<RefClassT>;
            typedef paramsFunctor<MethodT> FunctorT;
            enum
            {
                InvokeSelect = doInvokeBase::ivsMethod,
                ArgsCount = TraitsT::ArgsCount,
                Flags = (DefsT::IntSelect<DefsT::TypeEqual<ClassT, void>::Result,
                                          TJS_STATICMEMBER,
                                          0>::Result)
        ...
        struct InvokeCommand<REFCLASS, METHOD, ivtProxy<ivtNormal>>
          : public InvokeCommand<REFCLASS, METHOD>
        {
            typedef /*                                                      */ InvokeCommand<
                REFCLASS,
                METHOD>
                BaseT;
          ...
          : public InvokeCommand<REFCLASS, METHOD, ivtBridge<BRIDGE>>
        {
            ...
            typedef paramsFunctorWithInstance<typename BaseT::ClassT, typename BaseT::FunctorT>
                FunctorT;
            enum
            {
                InvokeSelect = doInvokeBase::ivsProxy,
...
struct ncbNativeClassMethod : public ncbNativeClassMethodBase
{
    ...
    {
        if (!_method)
            TVPThrowExceptionMessage(TJS_N("No method pointer."));
    }
                       ...
                       iTJSDispatch2* objthis)
    {
        ...
        if (membername)
            return BaseT::FuncCall(flag, membername, hint, result, numparams, param, objthis);
    ...
    /// factory
    static iMethodT Create(MethodT m, bool create = true)
    {
        return !create ? 0 : (new ThisClassT(m))->GetIMethod();
    }
...
struct ncbNativeClassConstructor : public ncbNativeClassMethodBase
{
    ...
    ncbNativeClassConstructor(MethodT m) : ncbNativeClassMethodBase(nitMethod), _method(m)
    {
        if (!_method && (int)CommandT::InvokeSelect == (int)doInvokeBase::ivsFactory)
            TVPThrowExceptionMessage(TJS_N("No factory pointer."));
    }
...
struct ncbNativeClassFactory : public ncbNativeClassMethodBase
{
    ...
    typedef tjs_error (*MethodT)(ClassT** result,
                                 tjs_int numparams,
                                 tTJSVariant** param,
                                 iTJSDispatch2* objthis);
...
struct ncbNativeClassProperty : public ncbNativeClassMethodBase
{
                      ...
                      iTJSDispatch2* objthis)
    {
        // 自分自身が呼ばれたのではない場合は丸投げ
        if (membername)
            return BaseT::PropGet(flag, membername, hint, result, objthis);
                      ...
                      iTJSDispatch2* objthis)
    {
        // 自分自身が呼ばれたのではない場合は丸投げ
        if (membername)
            return BaseT::PropSet(flag, membername, hint, param, objthis);
        ...
        // メソッド呼び出し
        return (doInvoke<SetCommandT>(_setter, 0, 1, const_cast<tTJSVariant**>(&param), objthis))
            .Invoke();
    }

    /// factory
    static iMethodT Create(GetterT g, SetterT s, bool create = true)
    {
        return !create ? 0 : (new ThisClassT(g, s))->GetIMethod();
    }
...
struct ncbRawCallbackMethod<
    /*        */ tjs_error (*)(
        tTJSVariant* result, tjs_int numparams, tTJSVariant** param, T* nativeInstance)>
  : public ncbNativeClassMethodBase
{
    typedef tjs_error (*CallbackT)(tTJSVariant* result,
                                   tjs_int numparams,
                                   tTJSVariant** param,
                                   T* nativeInstance);
    typedef T NativeClassT;
    ...
    {
        if (!_callback)
            TVPThrowExceptionMessage(TJS_N("No callback pointer."));
    }
    ...
    /// factory
    static iMethodT Create(CallbackT cb, FlagsT f, bool create = true)
    {
        return !create ? 0 : (new ThisClassT(cb, f))->GetIMethod();
    }
...
struct ncbRawCallbackMethod<tTJSNativeClassMethodCallback> : public ncbIMethodObject
{
    ...
    ncbRawCallbackMethod(CallbackT m, FlagsT f)
      : _dispatch(TJSCreateNativeClassMethod(m)),
        _flags(f)
    {
    }
    ...
    static iMethodT Create(CallbackT cb, FlagsT f, bool create = true)
    {
        return !create ? 0 : (new ThisClassT(cb, f));
    }
...
struct ncbRawCallbackProperty : public ncbNativeClassMethodBase
{
    ...
    /// factory
    static iMethodT Create(GetCallbackT get, SetCallbackT set, FlagsT f, bool create = true)
    {
        return !create ? 0 : (new ThisClassT(get, set, f))->GetIMethod();
    }
...
private:
    ...
    ivtBridge<METHOD> getBridgeType(METHOD) const
    {
        return ivtBridge<METHOD>();
    }
    template<class METHOD>
    ivtProxy<ivtBridge<METHOD>> getProxyBridgeType(METHOD) const
    {
        return ivtProxy<ivtBridge<METHOD>>();
    }
    ...
    struct MethodType
    {
        typedef typename _DefsT::CallerT::tMethodResolver<
            typename _DefsT::CallerT::tMethodTraits<METHOD>::ResultType,
            BASE,
...
public:
    ...
    _StringT GetName(OTHER s) const
    {
        return _StringT(s);
    }
    void Begin()
    ...
    void DoItem(_NameT n, _ItemT t)
    {
        if (_isRegist)
            _impl.RegistItem(n, t);
        else
            _impl.UnregistItem(n);
    }
    void DoItem(_StringT s, _ItemT t)
    {
        if (_isRegist)
            _impl.RegistItem(s.c_str(), t);
        else
            _impl.UnregistItem(s.c_str());
    }
    ...
    // 読み込み・書き込み専用プロパティ
    template<typename NAME, typename GetterT, class IVT>
    void Property(NAME n, GetterT g, int, IVT const& tag)
    {
        Property(n, g, static_cast<GetterT>(0), tag);
    }
    template<typename NAME, typename SetterT, class IVT>
    void Property(NAME n, int, SetterT s, IVT const& tag)
    {
        Property(n, static_cast<SetterT>(0), s, tag);
    }
    ...
    void Property(NAME n, GetterT g, SetterT s)
    {
        Property(n, g, s, Normal);
    }

    // Bridge 時
    template<typename N, typename M, class B>
    void Method(N n, M m, Bridge<B> const&)
    {
        Method(n, m, getBridgeType(&B::operator()));
    }
    template<typename N, typename M, class B>
    void Method(N n, M m, ProxyBridge<B> const&)
    {
        Method(n, m, getProxyBridgeType(&B::operator()));
    }
    template<typename N, typename M, class B>
    void Method(N n, M m, BridgeProxy<B> const&)
    {
        Method(n, m, getProxyBridgeType(&B::operator()));
    }
    template<typename N, typename G, typename S, class B>
    void Property(N n, G g, S s, Bridge<B> const&)
    {
        Property(n, g, s, getBridgeType(&B::operator()));
    }
    template<typename N, typename G, typename S, class B>
    void Property(N n, G g, S s, ProxyBridge<B> const&)
    {
        Property(n, g, s, getProxyBridgeType(&B::operator()));
    }
    template<typename N, typename G, typename S, class B>
    void Property(N n, G g, S s, BridgeProxy<B> const&)
    {
        Property(n, g, s, getProxyBridgeType(&B::operator()));
    }
    ...
    {
        DoItem(GetName(),
               ncbNativeClassConstructor<InvokeCommand<ClassT, MethodT, ivtCtor>>::Create(
                   0, _isRegist));
    }
    // デフォルトコンストラクタの登録
...
#define FOREACH \
    ...
    void Constructor(typename _DefsT::CallerT::tMethodType<void, _ClassT, \
                                                           FOREACH_COMMA_EXT(CTOR_ARG_EXT)>::Type) \
    { \
        /**/ Constructor(TypeWrap<typename _DefsT::CallerT::tMethodType< \
                             void, _ClassT, FOREACH_COMMA_EXT(CTOR_ARG_EXT)>::Type>()); \
    }
#include FOREACH_INCLUDE
#undef CTOR_PRM_EXT
#undef CTOR_ARG_EXT
    ...
    {
        DoItem(GetName(),
               ncbNativeClassConstructor<InvokeCommand<ClassT, MethodT, ivtFactory>>::Create(
                   m, _isRegist));
    }
    // RawCallback Factory
    ...
    void SubClass(NAME n, TypeWrap<CLASS>)
    {
        ...
        if (!SubClassT::Setup(GetName(n), _isRegist))
            TVPThrowExceptionMessage(TJS_N("SubClass registration failed."));
...
struct ncbRegistNativeClass : public ncbRegistNativeClassBase
{
    ...
    void RegistBegin()
    {
        NCB_LOG_2(TJS_N("BeginRegistClass: "), _className);
        ...
        if (!ClassInfoT::Set(_className, id, _classobj))
            TVPThrowExceptionMessage(TJS_N("Already registerd class."));

        // クラスオブジェクトにIDを設定
        TJSNativeClassSetClassID(_classobj, id);

        // 空のfinalizeメソッドを追加
        TJSNativeClassRegisterNCM(_classobj, TJS_N("finalize"),
                                  TJSCreateNativeClassMethod(EmptyCallback), _className, nitMethod);
    }
    ...
    {
        NCB_LOG_2(TJS_N("  RegistItem: "), name);
        if (name == _className)
        {
            if (_hasCtor)
                TVPAddLog(tTJSString(TJS_N("Multiple constructors.(")) + _className + TJS_N(")"));
            _hasCtor = true;
        }
        if (item)
        {
            TJSNativeClassRegisterNCM(_classobj, name, item->GetDispatch(), _className,
                                      item->GetType(), item->GetFlags());
            item->Release();
        }
    }
    ...
    void RegistEnd()
    {
        ...
        NCB_LOG_2(TJS_N("EndRegistClass: "), _className);
    }
    ...
    void UnregistEnd()
    {
        ...
        _RemoveClassInfo();
        NCB_LOG_2(TJS_N("EndUnregistClass: "), _className);
    }
    void UnregistBegin() { NCB_LOG_2(TJS_N("BeginUnregistClass: "), _className); }
...
private:
    ...
    static tjs_error EmptyCallback(tTJSVariant*, tjs_int, tTJSVariant**, iTJSDispatch2*)
    {
        return TJS_S_OK;
    }

    /// エラーを返すメソッド(empty Constructor用)
    static tjs_error NotImplCallback(tTJSVariant*, tjs_int, tTJSVariant**, iTJSDispatch2*)
    {
    ...
    void _AddDummyConstructor() const
    {
        if (!_hasCtor)
            TJSNativeClassRegisterNCM(_classobj, _className,
                                      TJSCreateNativeClassMethod(NotImplCallback), _className,
                                      nitMethod);
    }
    void _RemoveClassInfo() const
    {
        NCB_LOG_2(TJS_N("  RemoveClassInfo: "), _className);
        ClassInfoT::Clear();
    }
};
...
struct ncbRegistSubClass : public ncbRegistNativeClass<CLASS>
{
    ...
    void RegistEnd()
    {
        ...
        NCB_LOG_2(TJS_N("EndSubClass: "), BaseT::_className);
    }
    void UnregistEnd()
    {
        BaseT::_RemoveClassInfo();
        NCB_LOG_2(TJS_N("EndUnregistSubClass: "), BaseT::_className);
    }
};
...
struct ncbSubClassItem : public ncbIMethodObject
{
    ...
    static bool Setup(ncbTypedefs::NameT name, bool isRegist)
    {
        ...
        if (isRegist && ClassInfoT::GetClassObject())
            return false;
        DelegateT d(name);
        {
...
struct ncbAttachTJS2Class : public ncbRegistNativeClassBase
{
    ...
    ncbAttachTJS2Class(NameT nativeClass, NameT tjs2Class)
      : BaseT(nativeClass),
        _tjs2ClassName(tjs2Class)
    {
    }

    void RegistBegin()
    {
        NCB_LOG_2(TJS_N("BeginAttachTJS2Class: "), _className);
        ...
        // クラスIDを生成
        IdentT id = TJSRegisterNativeClass(_className);
        NCB_LOG_2(TJS_N("  ID: "), (tjs_int)id);

        // ncbClassInfoに登録
        if (!ClassInfoT::Set(_className, id, 0))
        {
            TVPThrowExceptionMessage(TJS_N("Already registerd class:"), ttstr(_className));
        }
    }

    void RegistVariant(NameT name, tTJSVariant const& val, FlagsT flg)
    {
        _tjs2ClassObj->PropSet(TJS_MEMBERENSURE | flg, name, 0, &val,
    ...
    void RegistItem(NameT name, ItemT item)
    {
        ...
        if (name == _className)
        {
            TVPThrowExceptionMessage(TJS_N("Constructor attached: "), ttstr(_className));
        }
    ...
    void RegistEnd()
    {
        if (_global)
            _global->Release();
        _global = 0;
        NCB_LOG_2(TJS_N("EndAttachClass: "), _className);
        // _tjs2ClassObj は NoAddRef なので Release 不要
    }

    void UnregistBegin()
    {
        NCB_LOG_2(TJS_N("BeginDetach: "), _className);
        // クラスオブジェクトを取得
        iTJSDispatch2* global = TVPGetScriptDispatch();
        if (global)
        {
    ...
    {
        NCB_LOG_2(TJS_N("DetachItem: "), name);
        if (_tjs2ClassObj)
        {
    ...
    {
        ClassInfoT::Clear();
        NCB_LOG_2(TJS_N("EndDetach: "), _className);
        // _tjs2ClassObj は NoAddRef なので Release 不要
    }
...
#define NCB_INNER_AUTOREGISTER_LINES_INSTANCE \
    ...
    {
#ifdef _DEBUG
        NCB_LOG_2(TJS_N("AllUnregist:"), line);
#endif
        for (ThisClassT const* p = _top[line]; p; p = p->_next)
            p->Unregist();
    }
    ...
    static void AllRegist()
    {
        for (int line = 0; line < LINE_COUNT; line++)
            AllRegist(static_cast<LineT>(line));
    }
    static void AllUnregist()
    {
        for (int line = 0; line < LINE_COUNT; line++)
...
struct ncbNativeClassAutoRegister : public ncbAutoRegister
{
    /**/ ncbNativeClassAutoRegister(NameT modulename, NameT n)
      : ncbAutoRegister(modulename, ClassRegist),
        _name(n)
    {
    }
...
#define NCB_REGISTER_CLASS_DELAY(name, cls) \
    NCB_REGISTER_CLASS_COMMON(cls, ncbNativeClassAutoRegister, (NCB_MODULE_NAME, TJS_N(#name)))
...
{
    /**/ ncbAttachTJS2ClassAutoRegister(NameT modulename, NameT ncn, NameT tjscn)
      : ncbAutoRegister(modulename, ClassRegist),
        _nativeClassName(ncn),
        _tjs2ClassName(tjscn)
    {
    }
...
protected:
    void Regist() const
    {
        DelegateT d(_nativeClassName, _tjs2ClassName);
        {
            RegistT r(d, true);
            r.Regist();
        }
...
{
    /**/ ncbRequireClassAutoRegister(NameT modulename, NameT name, NameT sub = 0)
      : ncbAutoRegister(modulename, ClassRegist),
        _className(name),
        _expName(sub)
    {
...
protected:
    void Regist() const
    {
        NCB_LOG_2(TJS_N("RequireClass: "), _className);

        // クラスオブジェクト取得
        tTJSVariant val;
        TVPExecuteExpression(ttstr(_expName ? _expName : _className), &val);
        if (val.Type() != tvtObject)
            TVPThrowExceptionMessage(TJS_N("Require class not found."));
...
#define NCB_GET_INSTANCE_HOOK(cls) \
    template<> \
    struct ncbNativeClassMethodBase::nativeInstanceGetter<cls> \
      : public ncbNativeClassMethodBase::nativeInstanceGetterBase<cls>

#define NCB_GET_INSTANCE_HOOK_CLASS nativeInstanceGetter

#define NCB_INSTANCE_GETTER(objthis) inline ClassT* Get(DispatchT objthis)
...
#define NCB_ATTACH_CLASS_WITH_HOOK(cls, attach) \
    ...
    NCB_REGISTER_CLASS_COMMON(cls, ncbAttachTJS2ClassAutoRegister, \
                              (NCB_MODULE_NAME, TJS_N(#cls), TJS_N(#attach)))

#define NCB_ATTACH_CLASS(cls, attach) \
    NCB_ATTACHED_INSTANCE_DELAY_CREATE(cls); \
    NCB_ATTACH_CLASS_WITH_HOOK(cls, attach)

#define NCB_REQUIRE_CLASS(cls) \
    template struct ncbClassInfo<cls>; \
    static ncbRequireClassAutoRegister<cls> ncbRequireClass_##cls(TJS_N(#cls), 0)
#define NCB_REQUIRE_SUBCLASS(cls, name) \
    template struct ncbClassInfo<cls>; \
    static ncbRequireClassAutoRegister<cls> ncbRequireClass_##cls(TJS_N(#cls), TJS_N(#name))

////////////////////////////////////////
#define NCB_METHOD_RAW_CALLBACK(name, cb, flag) RawCallback(TJS_N(#name), cb, flag)
#define NCB_PROPERTY_RAW_CALLBACK(name, get, set, flag) RawCallback(TJS_N(#name), get, set, flag)
#define NCB_PROPERTY_RAW_CALLBACK_RO(name, get, flag) RawCallback(TJS_N(#name), get, (int)0, flag)
#define NCB_PROPERTY_RAW_CALLBACK_WO(name, set, flag) RawCallback(TJS_N(#name), (int)0, set, flag)

#define NCB_CONSTRUCTOR(cargs) Constructor(TypeWrap<void(Class::*) cargs>())

#define NCB_METHOD_DIFFER(name, method) Method(TJS_N(#name), &Class::method)
#define NCB_METHOD(method) NCB_METHOD_DIFFER(method, method)

#define NCB_METHOD_CAST(tag, result, method, args) \
    static_cast<MethodType<tag, result(*) args>::Type>(&method) // tag = { Class, Const, Static }
#define NCB_METHOD_DETAIL(name, T, R, M, A) Method(TJS_N(#name), NCB_METHOD_CAST(T, R, M, A))

#define NCB_PROPERTY(name, get, set) Property(TJS_N(#name), &Class::get, &Class::set)
#define NCB_PROPERTY_RO(name, get) Property(TJS_N(#name), &Class::get, (int)0)
#define NCB_PROPERTY_WO(name, set) Property(TJS_N(#name), (int)0, &Class::set)

#define NCB_PROPERTY_DETAIL(N, RT, RR, RM, RA, WT, WR, WM, WA) \
    Property(TJS_N(#N), NCB_METHOD_CAST(RT, RR, RM, RA), NCB_METHOD_CAST(WT, WR, WM, WA))
#define NCB_PROPERTY_DETAIL_RO(name, RT, RR, RM, RA) \
    Property(TJS_N(#name), NCB_METHOD_CAST(RT, RR, RM, RA), (int)0)
#define NCB_PROPERTY_DETAIL_WO(name, WT, WR, WM, WA) \
    Property(TJS_N(#name), (int)0, NCB_METHOD_CAST(WT, WR, WM, WA))

// さあ何がなんだかわかんなくなってきました（汗
#define NCB_METHOD_PROXY(name, method) Method(TJS_N(#name), &method, Proxy)
#define NCB_METHOD_PROXY_DETAIL(name, T, R, M, A) \
    Method(TJS_N(#name), NCB_METHOD_CAST(T, R, M, A), Proxy)
#define NCB_PROPERTY_PROXY(name, get, set) Property(TJS_N(#name), &get, &set, Proxy)
#define NCB_PROPERTY_PROXY_RO(name, get) Property(TJS_N(#name), &Class::get, (int)0, Proxy)
#define NCB_PROPERTY_PROXY_WO(name, set) Property(TJS_N(#name), (int)0, &Class::set, Proxy)
#define NCB_PROPERTY_PROXY_DETAIL(N, RT, RR, RM, RA, WT, WR, WM, WA) \
        /*                                                */Property(TJS_N(# N),    NCB_METHOD_CAST(RT,RR,RM,RA), NCB_METHOD_CAST(WT,WR,WM,WA)), Proxy)
#define NCB_PROPERTY_PROXY_DETAIL_RO(name, RT, RR, RM, RA) \
    Property(TJS_N(#name), NCB_METHOD_CAST(RT, RR, RM, RA), (int)0, Proxy)
#define NCB_PROPERTY_PROXY_DETAIL_WO(name, WT, WR, WM, WA) Property(TJS_N(# name), (int)0,                       NCB_METHOD_CAST(WT,WR,WM,WA)), Proxy)

#define NCB_SUBCLASS(name, cls) SubClass(TJS_N(#name), TypeWrap<cls>())
...
protected:
    ...
    static void RegistFunction(NameT name, NameT attach, MethodT m)
    {
        ...
        typedef ncbNativeClassMethod<IVT::InvokeCommand<void, MethodT, IVT::ivtNormal>>
            MethodObjectT;
...
#define NCB_REGISTER_FUNCTION_COMMON(name, tag, attach, function) \
    ...
    template<> \
    struct ncbNativeFunctionAutoRegisterTempl<ncbFunctionTag_##tag> \
    ...
    { \
        ...
        void Regist() const \
        { \
            RegistFunction(TJS_N(#name), attach, &function); \
        } \
        void Unregist() const \
        { \
            UnregistFunction(TJS_N(#name), attach); \
        } \
    }; \
...
#define NCB_ATTACH_FUNCTION(name, attach, function) \
    NCB_REGISTER_FUNCTION_COMMON(name, attach##_##name, TJS_N(#attach), function)
#define NCB_ATTACH_FUNCTION_WITHTAG(name, tag, attach, function) \
    NCB_REGISTER_FUNCTION_COMMON(name, tag##_##name, TJS_N(#attach), function)
...
#define NCB_REGISTER_CALLBACK(pos, init, term, tag) \
    static ncbCallbackAutoRegister ncbCallbackAutoRegister_##pos##_##tag( \
