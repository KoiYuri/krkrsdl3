...
#define NCB_MODULE_NAME TJS_N("xp3filter.dll")
...
tjs_error CBinaryAccessor::OperationByNum(
    ...
    /* result ( can be NULL ) */ const tTJSVariant* param,
...
tjs_error CBinaryAccessor::Operation(
    ...
    /* member name ( NULL for a default member ) */ tjs_uint32* hint,
    ...
    /* parameters */ iTJSDispatch2* objthis /* object as "this" */)
{
    if (membername)
    {
        static const ttstr str_ptr(TJS_N("ptr"));
        if (hint)
        {
...
tjs_error CBinaryAccessor::IsValid(
    /* get validation, returns TJS_S_TRUE (valid) or TJS_S_FALSE (invalid) */ tjs_uint32 flag,
    /* calling flag */ const tjs_char* membername,
    /* member name ( NULL for a default member ) */ tjs_uint32* hint,
    /* hint for the member name (in/out) */ iTJSDispatch2* objthis /* object as "this" */)
{
...
tjs_error CBinaryAccessor::GetCount(
    ...
    /* variable that receives the result */ const tjs_char* membername,
    /* member name ( NULL for a default member ) */ tjs_uint32* hint,
    /* hint for the member name (in/out) */ iTJSDispatch2* objthis /* object as "this" */)
{
    if (membername)
...
tjs_error CBinaryAccessor::PropSet(/* property set */ tjs_uint32 flag,
                                   /* calling flag */ const tjs_char* membername,
                                   /* member name ( NULL for a default member ) */ tjs_uint32* hint,
                                   ...
                                   /* parameters */ iTJSDispatch2* objthis /* object as "this" */)
{
    if (!membername)
        return TJS_E_NOTIMPL;
    if (!TJS_strcmp(membername, TJS_N("ptr")))
    {
        m_curPos = param->AsInteger();
        return TJS_S_OK;
    }
...
tjs_error CBinaryAccessor::PropGetByNum(/* property get by index number */ tjs_uint32 flag,
                                        /* calling flag */ tjs_int num,
                                        /* index number */ tTJSVariant* result,
...
tjs_error CBinaryAccessor::PropGet(/* property get */ tjs_uint32 flag,
                                   /* calling flag */ const tjs_char* membername,
                                   /* member name ( NULL for a default member ) */ tjs_uint32* hint,
                                   /* hint for the member name (in/out) */ tTJSVariant* result,
                                   /* result */ iTJSDispatch2* objthis /* object as "this" */)
{
    if (!membername)
        return TJS_E_NOTIMPL;
    if (!TJS_strcmp(membername, TJS_N("count")))
    {
        result->operator=((tjs_int64)m_length);
    }
    else if (!TJS_strcmp(membername, TJS_N("ptr")))
    {
    ...
    /* parameters */ iTJSDispatch2* objthis /* object as "this" */)
{
    if (hint)
    {
        if (!*hint)
        {
            *hint = !TJS_strcmp(membername, TJS_N("xor"));
            if (*hint)
            {
    ...
    else if (!TJS_strcmp(membername, TJS_N("xor")))
    {
        return FuncXor(numparams, param);
    }
    return TJS_E_NOTIMPL;
...
#define REGISTER_OBJECT(classname, instance) \
    ...
    TJSNativeClassRegisterNCM(cls, TJS_N("setXP3ArchiveExtractionFilter"),
                              new XP3FilterRegister(decoder), cls->GetClassName().c_str(),
                              nitMethod, TJS_STATICMEMBER);
    TJSNativeClassRegisterNCM(cls, TJS_N("setXP3ArchiveContentFilter"),
                              new XP3ContentFilterRegister(decoder), cls->GetClassName().c_str(),
                              nitMethod, TJS_STATICMEMBER);
                                          ...
                                          tTJSVariant* ctx)
{
    ...
    tTJSVariant result;
    decoder->ManagedFilter.FuncCall(0, NULL, NULL, &result, sizeof(vars) / sizeof(vars[0]), vars,
                                    NULL);
    tjs_int ret = 0;
...
void TVPXP3ArchiveExtractionFilterWrapper(tTVPXP3ExtractionFilterInfo* info, tTJSVariant* ctx)
{
    if (info->SizeOfSelf != sizeof(tTVPXP3ExtractionFilterInfo))
        TVPThrowExceptionMessage(TJS_N("Incompatible tTVPXP3ExtractionFilterInfo size"));
    XP3FilterDecoder* decoder = FetchXP3Decoder();
    if (decoder->ManagedDecoder.Object)
    {
...
void TVPSetXP3FilterScript(ttstr content)
{
    ...
    else
    {
        ...
        TVPSetXP3ArchiveContentFilter(
            (tTVPXP3ArchiveContentFilter)TVPXP3ArchiveContentFilterWrapper);
    }
