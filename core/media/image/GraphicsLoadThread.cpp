...
tTVPTmpBitmapImage::tTVPTmpBitmapImage() : MetaInfo(NULL)
{
}
...
static int TVPLoadGraphicAsync_SizeCallback(void* callbackdata,
                                            tjs_uint w,
                                            tjs_uint h,
    ...
    do
    {
        ...
        if (cmd != NULL)
        {
            ...
            if (cmd->result_.length() > 0)
            {
                ...
                {
                    TVPPostEvent(cmd->owner_, cmd->owner_, eventname, 0, TVP_EPT_IMMEDIATE, 4,
                                 param);
                }
