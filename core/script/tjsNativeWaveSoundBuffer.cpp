...
//---------------------------------------------------------------------------
static tTJSNativeClass* _waveflagsclass = NULL;
iTJSDispatch2* TVPCreateWaveFlagsObject(iTJSDispatch2* buffer)
{
    if (!_waveflagsclass)
    {
        _waveflagsclass = new tTJSNC_WaveFlags();
        TJS_REGISTER_STATIC_HEAP(_waveflagsclass);
    }

    iTJSDispatch2* out;
    tTJSVariant param(buffer);
    tTJSVariant* pparam = &param;
    if (TJS_FAILED(_waveflagsclass->CreateNew(0, NULL, NULL, &out, 1, &pparam, _waveflagsclass)))
        TVPThrowInternalError;
...
{
    TVPRemoveDecodeSoundBuffer(this);
