...
public:
    ...
    {
        return GetContextCount() == 1 && TopLevelContext != NULL && !UsingPreProcessor;
    }
...
public:
    ...
    void SetText(tTJSVariant* result,
                 const tjs_char* text,
    ...
    void SetObjects(tTJSInterCodeContext* toplevel,
                    std::vector<tTJSInterCodeContext*>& objs,
                    int count)
    {
    ...
    void Compile(const tjs_char* text,
                 bool isexpression,
                 bool isresultneeded,
                 bool outputdebug,
