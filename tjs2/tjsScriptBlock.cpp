...
ttstr tTJSScriptBlock::GetLineDescriptionString(tjs_int pos) const
{
    ...
    {
        tjs_char ptr[128];
        TJS_snprintf(ptr, sizeof(ptr) / sizeof(tjs_char), TJS_N("0x%p"), this);
        name = ttstr(TJS_N("anonymous@")) + ptr;
    }

    return name + TJS_N("(") + ttstr(line) + TJS_N(")");
}
//---------------------------------------------------------------------------
...
void tTJSScriptBlock::SetText(tTJSVariant* result,
                              const tjs_char* text,
    ...
    try
    {
...

        {
            ...
            if (parsetime)
            {
                ...
                OutputDebugString(buf);
                TJS_nsprintf(buf, "GenNodeCode : %d (%d%%)", time_GenNodeCode,
                ...
                OutputDebugString(buf);
                TJS_nsprintf(buf, "  this_proxy : %d (%d%%)", time_this_proxy,
...
#ifdef TJS_DEBUG_PROFILE_TIME
        tjs_char str[100];
        TJS_sprintf(str, TJS_N("%d"), clock() - start);
...
ttstr tTJSScriptBlock::GetNameInfo() const
{
    ...
    {
        return ttstr(Name) + TJS_N("(line +") + ttstr(LineOffset) + TJS_N(")");
    }
}
//---------------------------------------------------------------------------
void tTJSScriptBlock::Dump() const
{
    std::list<tTJSInterCodeContext*>::const_iterator i = InterCodeContextList.begin();
    while (i != InterCodeContextList.end())
    {
        ConsoleOutput(TJS_N(""), (void*)this);
        tjs_char ptr[256];
        TJS_snprintf(ptr, sizeof(ptr) / sizeof(tjs_char), TJS_N(" 0x%p"), (*i));
        ConsoleOutput((ttstr(TJS_N("(")) + ttstr((*i)->GetContextTypeName()) + TJS_N(") ") +
                       ttstr((*i)->GetName()) + ptr)
                          .c_str(),
                      (void*)this);
        (*i)->Disassemble(ConsoleOutput, (void*)this);
...
//---------------------------------------------------------------------------
const tjs_uint8 tTJSScriptBlock::BYTECODE_FILE_TAG[BYTECODE_FILE_TAG_SIZE] = {'T', 'J', 'S', '2',
                                                                              '1', '0', '0', 0};
...
void tTJSScriptBlock::ExportByteCode(bool outputdebug, tTJSBinaryStream* output)
{
    ...
    objsize +=
        BYTECODE_TAG_SIZE + BYTECODE_CHUNK_SIZE_LEN + 4 + 4; // OBJS tag + size + toplevel + count
    std::vector<tjs_uint8>* dataarea = constarray->ExportBuffer();
    int datasize =
        (int)dataarea->size() + BYTECODE_TAG_SIZE + BYTECODE_CHUNK_SIZE_LEN; // DATA tag + size
...
//---------------------------------------------------------------------------
void tTJSScriptBlock::Compile(const tjs_char* text,
                              bool isexpression,
                              bool isresultneeded,
                              bool outputdebug,
