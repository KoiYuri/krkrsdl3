...
int _yyerror(const tjs_char* msg, void* pm, tjs_int pos)
{
    ...
    if (!TJS_strncmp(msg, TJS_N("syntax error"), 11))
    {
        str = TJSSyntaxError;
    ...
    tjs_char buf[43];
    TJS_snprintf(buf, sizeof(buf) / sizeof(tjs_char), TJS_N(" at line %d"),
                 1 + sb->SrcPosToLine(errpos));
    str += buf;
...
void tTJSExprNode::AddArrayElement(const tTJSVariant& val)
{
    ...
    Val->AsObjectClosureNoAddRef().FuncCall(0, ss_add.c_str(), ss_add.GetHint(), NULL, 1, args,
                                            NULL);
}
//---------------------------------------------------------------------------
...
#ifdef ENABLE_DEBUGGER
    // 古いローカル変数は削除してしまう
    TJSDebuggerClearLocalVariable(GetClassName().c_str(), GetName(), Block->GetName(),
...
//---------------------------------------------------------------------------
// for Byte code
tTJSInterCodeContext::tTJSInterCodeContext(tTJSScriptBlock* block,
                                           const tjs_char* name,
                                           tTJSContextType type,
                                           tjs_int32* code,
                                           tjs_int codeSize,
...
const tjs_char* tTJSInterCodeContext::GetContextTypeName() const
{
    ...
    {
        case ctTopLevel:
            return TJS_N("top level script");
        case ctFunction:
            return TJS_N("function");
        case ctExprFunction:
            return TJS_N("function expression");
        case ctProperty:
            return TJS_N("property");
        case ctPropertySetter:
            return TJS_N("property setter");
        case ctPropertyGetter:
            return TJS_N("property getter");
        case ctClass:
            return TJS_N("class");
        case ctSuperClassGetter:
            return TJS_N("super class getter proxy");
        default:
...
void tTJSInterCodeContext::OutputWarning(const tjs_char* msg, tjs_int pos)
{
    ...
    tjs_char buf[43];
    TJS_snprintf(buf, sizeof(buf) / sizeof(tjs_char), TJS_N(" line %d"),
                 1 + Block->SrcPosToLine(errpos));
    str += buf;
...
tjs_int tTJSInterCodeContext::PutCode(tjs_int32 num, tjs_int32 pos)
{
    ...
    if (CodeAreaSize >= CodeAreaCapa)
    {
        ...
        CodeArea =
            (tjs_int32*)TJS_realloc(CodeArea, sizeof(tjs_int32) * (CodeAreaCapa + TJS_INC_SIZE));
        if (!CodeArea)
...
tjs_int tTJSInterCodeContext::PutData(const tTJSVariant& val)
{
    ...
    if (_DataAreaSize >= _DataAreaCapa)
    {
        ...
        _DataArea = (tTJSVariant**)TJS_realloc(_DataArea, sizeof(tTJSVariant*) *
...
//---------------------------------------------------------------------------
int tTJSInterCodeContext::tSourcePos::SortFunction(const void* a, const void* b)
{
...
void tTJSInterCodeContext::FixCode(void)
{
    ...
    if (NonLocalFunctionDeclVector.size() >= 1)
    {
        ...
        // compute codesize
        tjs_int codesize = 2;
        for (func = NonLocalFunctionDeclVector.begin(); func != NonLocalFunctionDeclVector.end();
             func++)
        {
            if (func->ChangeThis)
                codesize += 10;
    ...
    for (fix = FixList.begin(); fix != FixList.end(); fix++)
    {
        ...
        if (fix->NewSize > fix->Size)
        {
            ...
            CodeArea = (tjs_int32*)TJS_realloc(
                CodeArea, sizeof(tjs_int32) * (CodeAreaSize + (fix->NewSize - fix->Size)));
            if (!CodeArea)
...
void tTJSInterCodeContext::Commit()
{
    ...
    {
        SourcePosArray =
            (tSourcePos*)TJS_realloc(SourcePosArray, SourcePosArraySize * sizeof(tSourcePos));
        if (!SourcePosArray)
            TJS_eTJSScriptError(TJSInsufficientMem, Block, 0);
...
{
    return Block->GetLineDescriptionString(CodePosToSrcPos(codepos)) + TJS_N("[") +
           GetShortDescription() + TJS_N("]");
}
//---------------------------------------------------------------------------
                                          ...
                                          const tSubParam& param)
{
    ...
    switch (node->GetOpecode())
    {
        ...
        case T_IF: // 'if' operator
        {
            ...
            tjs_int resaddr =
                _GenNodeCode(frame, (*node)[1], TJS_RT_NEEDED | TJS_RT_CFLAG, 0, tSubParam());
            bool inv = false;
        ...
        case T_QUESTION: // '?' ':' operator
        {
            ...
            {
                if ((restype & TJS_RT_NEEDED) && !TJSIsCondFlagRetValue(resaddr1) &&
                    !TJSIsFrame(resaddr1))
                {
                    PutCode(VM_CP, node_pos);
                    PutCode(TJS_TO_VM_REG_ADDR(frame), node_pos);
        ...
        case T_LOGICALAND: // '&&' operator
        {
            ...
            tjs_int resaddr1, resaddr2;
            resaddr1 =
                _GenNodeCode(frame, (*node)[0], TJS_RT_NEEDED | TJS_RT_CFLAG, 0, tSubParam());
            bool inv = false;
            if (!TJSIsCondFlagRetValue(resaddr1))
            {
                PutCode(VM_TT, node_pos);
        ...
        case T_GTOREQUAL:    // '>=' operator
        {
            ...
            return (restype & TJS_RT_CFLAG) ? (code2 == VM_SETNF ? TJS_GNC_CFLAG_I : TJS_GNC_CFLAG)
                                            : resaddr1;
        }
        ...
        case T_NEW:          // 'new' operator
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    PutCode(
                        TJS_TO_VM_REG_ADDR(res = (restype & TJS_RT_NEEDED) ? (framestart - 1) : 0),
        ...
        case T_ARG:
            ...
            if ((*node)[0])
            {
                ...
                if (n->GetOpecode() == T_EXPANDARG)
                {
                    // expanding argument
                    if ((*n)[0])
                        AddFuncArg(_GenNodeCode(frame, (*n)[0], TJS_RT_NEEDED, 0, tSubParam()),
                                   fatExpand);
                    else
        ...
        case T_LBRACKET: // '[ ]' operator
        {
            ...
            switch (param.SubType)
            {
                ...
                case stPreInc:
                case stPreDec:
                    PutCode((param.SubType == stPreInc ? VM_INC : VM_DEC) + (direct ? 1 : 2),
                            node_pos);
                    PutCode(TJS_TO_VM_REG_ADDR((restype & TJS_RT_NEEDED) ? frame : 0), node_pos);
                    PutCode(TJS_TO_VM_REG_ADDR(resaddr), node_pos);
                    PutCode(TJS_TO_VM_REG_ADDR(dp), node_pos);
                ...
                case stPostDec:
                {
                    ...
                    PutCode((param.SubType == stPostInc ? VM_INC : VM_DEC) + (direct ? 1 : 2),
                            node_pos);
        ...
        case T_ARRAYARG:
        {
            ...
            resaddr =
                (*node)[0] ? _GenNodeCode(frame, (*node)[0], TJS_RT_NEEDED, 0, tSubParam()) : 0;
...
void tTJSInterCodeContext::AddLocalVariable(const tjs_char* name, tjs_int init)
{
    ...
    if (Namespace.GetLevel() >= base)
    {
        ...
        if (init != 0)
        {
            ...
            TJSDebuggerAddLocalVariable(GetClassName().c_str(), GetName(), Block->GetName(),
                                        FunctionRegisterCodePoint, name, regoffset);
#endif // ENABLE_DEBUGGER
...
void tTJSInterCodeContext::DoBreak(void)
{
    ...
    {
        tNestData& data = NestVector[i];
        if (data.Type == ntSwitch || data.Type == ntWhile || data.Type == ntDoWhile ||
            data.Type == ntFor)
        {
...
#ifndef TJS_NO_CONSTANT_FOLDING
    if (node1 && node1->GetOpecode() == T_CONSTVAL && node2 && node2->GetOpecode() == T_CONSTVAL)
    {
        // constant folding
        switch (opecode)
        {
 ...
 * @return
 */
std::vector<tjs_uint8>* tTJSInterCodeContext::ExportByteCode(bool outputdebug,
                                                             tTJSScriptBlock* block,
